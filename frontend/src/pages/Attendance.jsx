...
export default function Attendance() {
  ...
  return (
    <div>
      <PageHeader title="Attendance" subtitle="Daily first-match logs generated by identification events" />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-[10px] uppercase tracking-[0.25em] text-white/40 font-mono border-b border-white/10">
                <th className="px-4 py-3">Identity</th><th className="px-4 py-3">Employee</th><th className="px-4 py-3">Department</th><th className="px-4 py-3">Camera</th><th className="px-4 py-3">Similarity</th><th className="px-4 py-3">Timestamp</th>
              </tr>
            </thead>
            <tbody className="font-mono">
              {rows.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-white/40">No attendance yet.</td></tr>}
              {rows.map((r) => (
                <tr key={r.id} className="border-b border-white/5 hover:bg-[#1A1A1A]">
                  ...
                  <td className="px-4 py-3 text-white/70 text-xs">{r.department || "—"}</td>
